...
def get_or_create_cart(request):
    ...
    cart = Carrito.objects.filter(cart_id=cart_id).first() #filter retorna una lista con los elementos .first() retorna el 1ro, si no hay None

    if cart is None:
        cart = Carrito.objects.create(user=user)
